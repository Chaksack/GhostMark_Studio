...
// Medusa v2 returns in MAJOR currency units (35 means £35.00). See the
// header of `~/utils/money` for the evidence and for why the previous
// divide-by-100 here was wrong. Amounts are rendered to a bare 2dp decimal
// via `toAmountString` (schema.org wants an unformatted number, not a
// currency-styled string), so this file shares the storefront's single
// money contract without pulling in currency symbols.
//
...
export function useProductSchema(input: UseProductSchemaInput): {
  ...
  const schema = computed<ProductSchema | null>(() => {
    ...
    if (isPOD && prices.length > 1 && lowPrice !== null && highPrice !== null) {
      offers = {
        '@type': 'AggregateOffer',
        lowPrice: toAmountString(lowPrice)!,
        highPrice: toAmountString(highPrice)!,
