variable "env" { type = string description = "Environment name, e.g., prod" }
...
variable "vpc_cidr" { type = string default = "10.20.0.0/16" description = "CIDR block for the VPC" }
variable "public_subnet_cidrs" { type = list(string) default = ["10.20.1.0/24", "10.20.2.0/24", "10.20.3.0/24"] description = "CIDRs for public subnets (ALB)" }
variable "azs" { type = list(string) default = ["eu-west-2a", "eu-west-2b", "eu-west-2c"] description = "Availability Zones used" }
...
variable "upload_logs_bucket_name" { type = string description = "S3 bucket name to store access logs for uploads bucket" }

# CORS for uploads bucket (restrict in prod)
variable "allowed_origins" { type = list(string) default = null description = "Allowed CORS origins for the uploads bucket; null to disable CORS config" }
...
variable "rds_instance_class" { type = string default = "db.t4g.small" description = "RDS instance class" }
variable "rds_allocated_storage" { type = number default = 50 description = "Allocated storage (GiB)" }
...
variable "rds_multi_az" { type = bool default = true description = "Enable Multi-AZ" }
variable "rds_backup_retention" { type = number default = 14 description = "Backup retention in days" }
...
variable "monitoring_enabled" { type = bool default = true description = "Deploy Prometheus/Alertmanager on ECS" }
